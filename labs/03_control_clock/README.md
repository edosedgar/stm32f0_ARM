...
Что здесь происходит в подробностях станет ясно позже, пока лишь можно сказать, что этот код настраивает один вывод в модуле портов ввода-вывода. После этого можно будет управлять светодиодом. Добавим вызов функции в main.
...
__attribute__((naked)) static void delay(void)
{
    ...
    asm ("pop {r7, pc}");     // Выгружаем LR и R7 в регистры PC и R7, тем самым возвращаясь в место вызова функции
